...
class Body {
	...
	async formData() {
		...
		const {toFormData} = await Promise.resolve().then(function () { return require('./multipart-parser-93883114.js'); });
...
const fetchJson = async (path$1) => {
  try {
    const data = await fetch(path$1);
    const text = await data.text();
    await promises.mkdir(path.resolve(docsPath, "source/createUnrefFn")).catch((err) => {
      console.log(err);
    });
    promises.writeFile(path.resolve(docsPath, "source/createUnrefFn/index.md"), text);
  } catch (error) {
    errorTip("\u7F51\u7EDC\u8BF7\u6C42\u5931\u8D25\uFF01");
  }
};
