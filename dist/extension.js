...
const fetchFile = async (path$1, dir) => {
  try {
    const data = await fetch(path$1);
    const text = await data.text();
    await promises.mkdir(path.resolve(docsPath, dir)).catch(() => {
    });
    promises.writeFile(path.resolve(docsPath, `${dir}/index.md`), text);
  } catch (error) {
    errorTip("\u7F51\u7EDC\u8BF7\u6C42\u5931\u8D25\uFF01");
  }
};
...
const updateDocs = async () => {
  const config = await getConfigJson(path.resolve(`${docsPath}/config.json`));
  if (!config)
    return;
